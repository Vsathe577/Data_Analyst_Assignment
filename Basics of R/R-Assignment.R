...
2.  What are the different data objects in R ? and write syntax and example for each and every object.size()
a.vector
x<-c(1,2,3)
b.Matrix
...
x
d.List
List<-list(id=c(1,2,3,4),name=c("Rohan","Ram"),marks=c(98,97))
List
e.Dataframe
dataframe<-data.frame(id=c(1,2,3),name=c("Tom","MOhan","Ramesh"),marks=c(98,99,97))
dataframe
f.Factor
x<-c("5","4","5","6","6","4","4")
class(x)
